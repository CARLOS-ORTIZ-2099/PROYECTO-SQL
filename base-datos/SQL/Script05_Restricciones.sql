...
/* MOTORES DE TABLAS */
/* Estos motores de bases de datos definen cómo se guardaran los datos
   por ejemplo en archivos csv, tablas planas, etc.
   En mysql las más usadas son 2 : 
   - MyISAM  => esta guarda los datos como tablas planas, haciendo casi
   imposible hacer uniones o transacciones en ella
   - InnoDB (la más usada)=> esta permite guardar los datos de tal manera 
   que se puedan hacer uniones, transacciones, etc.
   Así mismo recalcar que en versiones más modernas de mysql/mariaDb el motor
   que viene por defecto es InnoDB
*/
...
/* se recomienda que la insercion se del tipo RESTRICT, esto por qué
   no sería ideal eliminar un registro que sirve como foreign key
   (dependencia) de otra tabla, de esta forma mantenemos la integridad
   de los datos
   se sugiera que las actualizaciones si sean del tipo CASCADE 
*/
...
/* ENCRIPTACIÓN Y DESENCRIPTADO DE CONTRASEÑAS */
